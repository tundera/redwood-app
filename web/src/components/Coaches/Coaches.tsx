			...
			<tbody>
				{coaches.map((coach) =>
					<tr key={coach.id}>
						...
						<td>
							{timeTag(coach.createdAt)}
						</td>
						<td>
							{timeTag(coach.updatedAt)}
						</td>
						<td>
							{truncate(coach.handle)}
						</td>
						<td>
							{truncate(coach.name)}
						</td>
						...
						<td>
							{truncate(coach.isAssistant)}
						</td>
						<td>
							{truncate(coach.teamId)}
						</td>
						<td>
							<nav className="rw-table-actions">
								<Link to={routes.coach({id: coach.id})}
								title={`Show coach ${coach.id} detail`}
								className="rw-button rw-button-small">
									Show
								</Link>
								<Link to={routes.editCoach({id: coach.id})}
